class Title {
    ...
    constructor ($) {
        ...
        this.originalTitle = $('[data-testid="hero-title-block__original-title"]').text().trim() || null
        ...
        this.poster = $('[data-testid="hero-media__poster"] img').first().attr('srcset').split(', ').pop().trim().split(' ')[0]
        ...
        this.language = $('[data-testid="title-details-languages"] li').map(function () {
            return $(this).text().trim()
        }).toArray()
        
        this.seasons = $('#browse-episodes-season option').filter(function () {
